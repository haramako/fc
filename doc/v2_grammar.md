...
[archive/go_evolution_plan.md](archive/go_evolution_plan.md) F-fmt の「文法 v2 の設計」項。
...
- **P4. AST は共通**（archive/v2_plan.md C2）。v1/v2 の差はパーサの受理範囲と sema の名前解決規則だけ。
...
### 3.9 レキサの是正 [実装済み 2026-09-14・バージョン非依存]

v1/v2 共通のレキサで直す（直しても正しいプログラムの意味は変わらない。archive/v2_plan.md §2.5 の保存項目）:

- `0b2` のような不正な 2 進数リテラル → エラー ✅（v1 は Ruby `to_i` 準拠で `0` に丸めていた）
- `\xZZ` の不正エスケープ → エラー ✅（v1 は 0）
- 数値リテラルの `_` 区切り（`1_000`、`0xab_cd`）を許す ✅（桁の間だけ。`1_` や `0x1__0` はエラー）
- `//` の直後に改行が来る空コメントの取り扱いは新レキサで既に正しい
- コーパス（castle / miku / fclib / test）に該当するリテラルは無く、golden は不変
