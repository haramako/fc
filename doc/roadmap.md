...
- [ ] 高: 実行時の変数を要素に持つ配列リテラルが変数のアドレスになる / panic / ca65 のエラー（`[n, m]`。実行時に組み立てる）
- [ ] 高: switch の case に文字列（`case "A":`）で panic
- [ ] 高: 範囲外の整数リテラルが正規化されず -O で結果が変わる（`var y:i8 = 200`）。範囲外の定数はエラーに
- [ ] 高: インライン展開した関数の中のローカル const 表で ca65 の `Symbol undefined`
- [ ] 高: ローカルの `var a:[?]u8 = [1, 2, 3]` の領域が確保されない
- [ ] 高: 名前付きの文字列定数を slice にすると終端の 0 が入る（`const NM = "joe"` の `@len` が 4）
- [ ] 高: ポインタの加減算がバイト単位（`p += 1` が要素の大きさを掛けない。`p[1]` と `*(p + 1)` が違う）
- [ ] 高: NES の `stdio.print_int16` が壊れている（fastcall の渡し方、16 進の表の D）
- [ ] 高: `@(address:)` / fc.toml の `[ram.*]` と fc の ZP / SRAM の重なりを検出しない（OAM を $0200 に置くと FC_FARCALL と重なる）
- [ ] 高: `@(farcall)` を忘れると切替バンクの関数を `jsr` で呼んで黙って壊れる
- [ ] 中: スカラーの const の型の注釈が無視される（`const B:u8 = 300` が u16）
- [ ] 中: struct・配列・ポインタに算術・順序比較が通る
- [ ] 中: `const X:[4]u8 = [1, 2]` の長さが無視される（ローカル var・struct のフィールドと扱いが違う）
- [ ] 中: 素のブロック `{ }` がスコープを作らない
- [ ] 中: u16 の可変量シフトが -O で通ったり通らなかったり
- [ ] 中: `use` の検索が作業ディレクトリ基準（ソースのディレクトリにする）
- [ ] 中: `mem.zero` / `mem.compare` / `stdio.ppu_put` の長さ 0 が 256 バイト
- [ ] 中: printf が slice・struct を黙って捨てる、符号付きを符号なしで出す
- [ ] 中: `@(symbol: "_interrupt")` の NMI のフレームが main の呼び出しと重なる（interrupt を暗黙に、両方から届く関数を警告）

### 言語: 整数と式

- [ ] 高: 整数の拡張（`var d:u16 = a + b` が 8 ビットで折り返す）。できなければ 16 ビットの文脈への流れ込みを警告
- [ ] 高: 符号の混在（u8 + i8 が i8 になる）。1 段広い型にするか警告し、規則を文書に
- [ ] 高: 暗黙の縮小（範囲外の定数はエラー、変数の縮小は警告か文書を直す）
- [ ] 高: 必ず真・偽になる比較を警告（`i < 256`（u8）の無限ループ、`hp - dmg < 0`）
- [ ] 高: 文字リテラル `'A'`（u8 の定数。migrate で fc 2 の `'…'` を `"…"` に）
- [ ] 中: 条件式（`c ? a : b`）
- [ ] 中: `as bool` / bool への代入を `!= 0` に
- [ ] 中: 定数添字の範囲外をエラーに（値が飛び飛びの enum の表も）
- [ ] 低: `if (x = 0)` の警告、enum の `++` / `--`、const 表の定数添字を定数に（`MONS[1].hp`）

### 言語: 宣言・データ

- [ ] 高: 未知・無効な属性をエラーに（`@(adress:)` が黙って RAM 変数になる）
- [ ] 中: グローバル変数の初期値（既存の項目「グローバル変数の初期化」）
...
- [ ] 中〜高: do-while（少なくとも専用のエラー）
- [ ] 中〜高: 後ろに case が続く空の case を警告（`case 2, 3:` を案内）
- [ ] 中: return 忘れの検査を広げる（`while (true)`、全メンバーの enum の switch、`@if`）
- [ ] 中: 初期化していないローカル変数の読み出しを警告
...
- [ ] 高: 割り込みの定型文を不要に（未定義なら空のハンドラ、stdio の NMI を差し替え可能に）
...
- [ ] 低: 文書が実装より古い所（struct の `==`、u16 の添字、代入の大きさ、ポインタ演算、ZP の配置。v3_plan.md §10.6）
