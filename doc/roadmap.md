...
計測基盤は [../bench/](../bench/README.md)（`go test ./bench`）。現状 fc は cc65 の 1.3〜2.2 倍、Oscar64 の 4〜10 倍遅い
（bench/README.md の比較表）。README に書いた 5 つの原因の順に潰す:

- [ ] A / X / Y と直前のストア先の追跡による冗長 `lda` / `sta` の除去（一時変数経由のコピー、`sta L; lda L`）
- [ ] ポインタ参照で `L`（ZP）を `reg` に写さず `(L+n),y` を直接使う
- [ ] ジャンプの連鎖・ループ末尾の条件分岐（`jmp @end; @end: jmp @begin` → 反転して `bcc @begin`）
- [ ] 8 ビットのループ変数の `dec` / `inc`
