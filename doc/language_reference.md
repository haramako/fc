...
| `bool` | 1 | | `true` / `false`、比較演算（`==` `!=` `<` …）と論理演算（`&&` `\|\|` `!`）の結果の型。値は 0 / 1。整数（`uint8`）と互換で、`if` の条件や `&`、整数の引数・戻り値・代入にそのまま使える（`var f = a < b;` の `f` は `bool`。`int` にしたいなら `var f:int = a < b;`） |
| `void` | 0 | | 戻り値なし |
...
- `options(inline: true)`: 呼び出しをその場に展開する（`jsr`/`rts` と引数の受け渡しが消え、展開先で最適化される。
  ...
  他の呼び出しの引数の中（`g(f(x))` の `f`）、関数ポインタ経由。どこからも呼ばれなくなった関数は出力されない（§4.7）
