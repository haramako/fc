...
- 二項演算・比較の型は両辺の大きいほう、同じ大きさなら符号付き。ただし**型のない定数**（整数リテラル、型を書かない `const`、
  `sizeof` / `@len` などとそれらの演算）は相手の型に合わせる（2026-09-27）: `vx < 0`（vx:i8）は i8、`w + 5`（w:u16）は u16。
  定数のほうが大きい型なら広いほうで計算する（`x * 300`、`0x2000 + y * 32` は u16）。大きさは相手以下で符号のために収まらない定数は、
  **演算では相手の型に切り詰め**（`x + -1`（x:u8）は u8 の `x + 255`）、**比較ではエラー**（`s < 200`（s:i8）、`x == -1`（x:u8）は
  `200 does not fit in i8`。以前は 200 が -56 に、-1 が 255 になっていた）。`5 as u16` や `const N:u8 = 5` は型付きの定数で、
  普通の値と同じ規則
...
- 値コピー: 代入 `a = b`、引数（値渡し）、戻り値はフィールド全体をコピーする
- 比較: struct・配列の `==` / `!=` はバイトの比較（パディングは無い）。bool は正規化しないので、`5 as bool` のように 0 / 1 以外の
  bool のフィールドがあると、真理値としては等しくても偽。slice のフィールドは先頭と長さの比較（中身が同じでも別の配列なら偽）。
  それ以外の演算（算術・ビット演算・`<` など・`!`・条件）は struct・配列（slice を含む）には使えない
