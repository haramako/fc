...
### 6.3 R3-c 採番の決定性（C5）（0.5 日）— G2 の例外 (b) ✅ 2026-09-12

ラベル名だけが変わり ROM バイトは変わらない作業。**2 コミットに分ける**:

1. [x] `golden_test.go` に `normalizeLabels(asm string) string` を追加: `@<name>_N` / `@N` / `$N` を
...
2. [x] `sema` の tmp/label カウンタを Lambda 単位（または Module 単位）に、`codegen` のラベルカウンタを Module 単位に。
...
- 結果:
  - コミット 1: `normalizeLabels`（`@<name>_N` / `@N` / `_DN` / `_N` / `__N` を出現順の通し番号へ）を asm 比較に導入。現行出力に恒等
  - コミット 2: 連番を `ir.Module.Seq`（モジュール単位）に。LLC のラベル `@N` は元からモジュール単位だった。
    **無名関数のシンボルは `_<mod>_$N`（マングル後 `_<mod>__DN`）にモジュール名で修飾**した —
    モジュール単位の連番では別モジュールの無名関数同士で `_D2` が衝突しうる（export シンボル）ため。
