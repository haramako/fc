...
> 詳細化した。R0〜R3 の進行管理はそちらが正典（**R0 は 2026-09-12 完了**）。本書は全体像・背景・Part B（機能追加）の正典として残す。
...
- [ ] 文法バージョン宣言（表記は v2_plan.md §7 で決定）。宣言なし = v1。パーサはバージョンでディスパッチ、
      **AST は共通**。goyacc は v1/v2 で文法ファイルを分けるか、スーパーセット + バージョンゲートかを
      文法差分の大きさで決める
- [ ] 文法 v2 の設計: 言語側の変更と **`use` の文法/セマンティクス変更（F-mod 用）を同じバージョンに同梱**する
...
- [ ] `fcc migrate`: v1 で読み → 共通 AST → v2 プリンタで出力。castle / miku / fclib / test を移行し、
      移行前後で asm が一致（または意図した差分のみ）であることを確認
- [ ] 複数バージョンの並存期間: v1 パーサを残す期間と削除条件を決める
...
- [ ] 依存グラフの構築と永続化。相互 `use` の扱いは v2 の `use` 設計に従う（DAG 強制か、宣言フェーズ分離か）
