...
| ローカル配列がだめ | **未修正・根本原因判明** | `var la:int[4]` が `L+0..3` に置かれるのに、添字計算の一時ポインタが `L+2..3` に割り付けられて配列を壊す（`la[2]`/`la[3]` が化ける。グローバル配列は正常）。レジスタ割付が配列サイズを見ていない（memo.txt「indexなどのsize倍する処理」） |
