...
- [ ] 本格的な動作テスト: 正確性が必要になったら外部ヘッドレスエミュレータの統合を検討。
      候補: Mesen 2 の testrunner モード (Lua自動化、最高精度) / libretro コア
      (QuickNES・Nestopia 等、C ABI でヘッドレス組み込み向き) / FCEUX (Lua)。
      internal/nes は依存ゼロの常時CIスモークとして残し、外部エミュは精度が要る
      検証 (タイミング・APU・スプライト描画) 用の任意テストにする二段構え
