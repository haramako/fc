...
- 使われない関数を出力しない（tree shaking、2026-09-15 案）。汎用ライブラリで「使う関数だけビルドに含めたい」が動機。
  ld65 だけでやるなら ar65 のライブラリ + 関数ごとに .o に分けるしかない（ld65 はオブジェクト単位でしか引き込まない、
  weak は cfg の SYMBOLS の数値だけ）が、fc は `use` を辿って全体を一度にコンパイルしているので fc 側で届かない関数を消す方が筋がいい。
  - 根: `main`、アドレスを取られた関数（関数ポインタ、`options(address:)`）、割り込みハンドラ、asm から参照される関数
  - 根から呼び出しグラフ（far call 判定で既に辿っている）を辿り、届かなかった関数は asm を出さない
  - asm からの参照は fc が `include("*.asm")` の中身を見ていないので分からない。`options(keep: true)` を付けるか、
    include した asm を grep して `_module_name` 形のシンボルを根に足す（見逃しても ld65 の Unresolved external で分かる）
  - 半日程度
