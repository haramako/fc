...
func TestFormatStyle(t *testing.T) {
	tests := []struct {
		...
		{"fc 3 attributes",
			"#fc 3\n@(bank:3,farcall);\nvar v:u8 @( address:0x2000 );\nfunction f():u8 @(inline,segment:\"g\"){return 1;}\n@(bss:\"X\"){var b:u8;}\n",
			"#fc 3\n@(bank: 3, farcall);\nvar v:u8 @(address: 0x2000);\nfunction f():u8 @(inline, segment: \"g\")\n{\n\treturn 1;\n}\n@(bss: \"X\") {\n\tvar b:u8;\n}\n"},
		{"fc 3 static if",
			"#fc 3\n@if(common.DEBUG){use dbg;}else @if (X>2) {use a;} else {use b;}\nconst D=false @(build);\n",
			"#fc 3\n@if (common.DEBUG) {\n\tuse dbg;\n} else @if (X > 2) {\n\tuse a;\n} else {\n\tuse b;\n}\nconst D = false @(build);\n"},
		{"fc 3 enum",
			"#fc 3\npublic enum State:u8{Stand,Jump=2,Die}\nfunction f():void{ s=.Jump; }\n",
			"#fc 3\npublic enum State:u8 {\n\tStand,\n\tJump = 2,\n\tDie,\n}\nfunction f():void\n{\n\ts = .Jump;\n}\n"},
		{"fallthrough",
			"#fc 3\nfunction f(x:u8):void{switch(x){case 1: x=2;fallthrough ;case 2: x=3;}}\n",
			"#fc 3\nfunction f(x:u8):void\n{\n\tswitch (x) {\n\tcase 1:\n\t\tx = 2;\n\t\tfallthrough;\n\tcase 2:\n\t\tx = 3;\n\t}\n}\n"},
		{"slice",
			"#fc 3\nconst T:[?]u8=[1,2];\nfunction f(s:[]const u8, t:[]*u8, w:[ : u16 ]const u8, x:[:u16]u16):void{ var a=T[ 1 .. ]; var b=s[..]; var c=s[i..j+1]; var d=T[..2]; }\n",
			"#fc 3\nconst T:[?]u8 = [1, 2];\nfunction f(s:[]const u8, t:[]*u8, w:[:u16]const u8, x:[:u16]u16):void\n{\n\tvar a = T[1..];\n\tvar b = s[..];\n\tvar c = s[i..j + 1];\n\tvar d = T[..2];\n}\n"},
		...
		{"fc 2 c-style for and incdec",
			"#fc 2\nfunction f():void { for(var i:u8=0;i<10;i++){ x++; } for(;;){ --y; } for(i=0; i<3; i+=1){} }\n",
			"#fc 2\nfunction f():void\n{\n\tfor (var i:u8 = 0; i < 10; i++) {\n\t\tx++;\n\t}\n\tfor (;;) {\n\t\t--y;\n\t}\n\tfor (i = 0; i < 3; i += 1) {}\n}\n"},
		{"fc 2 trailing comma kept only when multi-line",
			"#fc 2\nconst a = [1, 2,];\nconst b = [\n\t1, 2,\n\t3,\n];\nfunction f():void { g(1,); g(\n\t1,\n\t2,\n); }\n",
			"#fc 2\nconst a = [1, 2];\nconst b = [\n\t1, 2,\n\t3,\n];\nfunction f():void\n{\n\tg(1);\n\tg(\n\t\t1,\n\t\t2,\n\t);\n}\n"},
		{"fc 2 prefix types and casts",
			"#fc 2\nvar a:[4]*int;\nvar p:*[4]int;\nvar t:[]fn(int, *int):void;\nfunction f(x:i8):fn(a:int):int { var y = x as int * 2; var q = bitcast<*u16>(p); return f; }\n",
			"#fc 2\nvar a:[4]*int;\nvar p:*[4]int;\nvar t:[]fn(int, *int):void;\nfunction f(x:i8):fn(a:int):int\n{\n\tvar y = x as int * 2;\n\tvar q = bitcast<*u16>(p);\n\treturn f;\n}\n"},
